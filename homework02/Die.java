...
public class Die {
   ...
   */
   public Die( int nSides ) {
    if ( nSides < MINIMUM_SIDES) {
      throw new IllegalArgumentException("Die must have 4 or more sides, baby.");
    }
    this.sides = nSides;
    this.pips = (int) Math.floor(Math.random() * nSides + 1);
   }
   ...
   */
   public int roll() {
      this.pips = (int) Math.floor(Math.random() * sides + 1);
      return this.pips;
   }
